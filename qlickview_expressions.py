...
class QlikViewExpressionYamlToTab():
    ...
    def parse_expression_file(self, path, name, text, sio):
        expression = {}
        linenum = 0
        print 'Here'  
        def process_expression(exp):
            ...
            sio.write('\t')
            print exp
            print exp.get('def','undefined')
            sio.write(exp.get('def','undefined'))
            sio.write('\t')
    ...
    def regenerate_tab_file_content(self, path, onload=False):
        print 'regenerate_tab_file_content'
    ...
    def regenerate_expression_tab_file(self, path, onload=False, force=False):
        print 'regenerate_expression_tab_file'
...
class QlikViewExpression(sublime_plugin.EventListener):
    ...
    def on_post_save(self, view):
        ...
        if (fn.endswith(EXT_QLIKVIEW_EXPRESSION)):
            generator = QlikViewExpressionYamlToTab()
            generator.regenerate_expression_tab_file(view.file_name())
